...
def adjust_orientation(lattice):
    ...
    v2 = cross(v3,v1) # left-hand coordination!
...
#             print(f'atoms_2_1 = atoms_1 = {atoms_2}')
            atoms_2[:,0] = - atoms_2[:,0] + 1 #　ここでx方向で反転
#             print(f'atoms_2_2 = {atoms_2}')
            atoms_c, elements_c = atoms_2.copy(), elements_2.copy() 
            elements_c = elements_c[atoms_c[:,0] + 0.000001 > 1]
            atoms_c = atoms_c[atoms_c[:,0] + 0.000001 > 1] # atom_c →　atoms of center 
        ...
        xx, yy, zz = lattice_bi[0][0].copy(), lattice_bi[1][1].copy(), lattice_bi[2][2].copy()
        xhi, yhi, zhi = max(0, xx), max(0, yy), max(0, zz)
        xlo, ylo, zlo = xhi - abs(xx), yhi - abs(yy), zhi - abs(zz)    
        xy = lattice_bi[:,1][0].copy()
        xz = lattice_bi[:,2][0].copy()
        yz = lattice_bi[:,2][1].copy()
        
        self.xhi, self.yhi, self.zhi = xhi, yhi, zhi
        self.xlo, self.ylo, self.zlo = xlo, ylo, zlo
